...
public class PanelCategoria implements ActionListener, 
									KeyListener,
	...
	private JLabel tituloFuncion = new JLabel("MANTENIMIENTO DE CATEGORÍAS");
	private JLabel etiquetaNuevo = new JLabel("Nueva categoría: ");
	...
	public PanelCategoria(JFrame marco, Controlador controlador) {
	...
	public void keyPressed(KeyEvent ke) {
		...
		if(KeyEvent.getKeyText(ke.getKeyCode()).equals("Enter")) {
			...
			if(!modoEdicion) {
				...
				modificarRegistro();
				deshabilitarPanelEdicion();
				etiquetaNuevo.setText("Nueva Categoría: ");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		switch(ae.getActionCommand()) {
			...
			case "Editar":
				...
				deshabilitarPanelCRUD();
				etiquetaNuevo.setText("Editar Categoría: ");
	...
	private void modificarRegistro() {
		...
		String feedBack = controlador.actualizarCategoria((int)tabla.getValueAt(tabla.getSelectedRow(), 0), textoNuevo.getText());
		//
		if (feedBack.equals("Modificación de categoría correcta.")) {
	...
	private void modificarRegistro(int claveModificacion, String valorModificacion, int fila, int columna) {
		...
		String pregunta = "Ha modificado el nombre de la categoría cuyo código es: " + claveModificacion + "\n¿Está seguro?";
		String feedBack = "No se ha modificado la categoría.";
		if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
			feedBack = controlador.actualizarCategoria(claveModificacion, valorModificacion);
		}
		else {
	...
	private void borrarRegistro() {
		...
		String pregunta = "Va a borrar la categoría con código: " + tabla.getValueAt(tabla.getSelectedRow(), 0) + "\n¿Está seguro?";
		String feedBack = "No se ha borrado la categoría.";
		if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
			feedBack = controlador.eliminarCategoria((int)tabla.getValueAt(tabla.getSelectedRow(), 0));
			//
			if (feedBack.equals("Se ha borrado la categoría.")) {
	...
	private void registrarNuevo() {
		...
		informarUsuario(controlador.insertarCategoria(textoNuevo.getText()));
