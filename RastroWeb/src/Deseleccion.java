...
@WebServlet("/Deseleccion")
public class Deseleccion extends HttpServlet {
	//
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//UPDATE
		//SE DECREMENTA LA CANTIDAD DE UNIDADES DEL PRODUCTO, Y
		//SI ES MENOR QUE 1, SE ELIMINA DE LA SESIÓN
		Producto p = (Producto) pedido.getAttribute(oferta);
		
		if(p.getCantidad() == 1) {
			
			pedido.removeAttribute(oferta);
			
		}else {
			
			p.setCantidad(p.getCantidad() - 1);
			//SE MODIFICA EL REGISTRO EN LA SESIÓN
